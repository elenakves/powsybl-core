...
class SetTargetPDeltaExtension implements DslTaskExtension {

    @Override
    void addToSpec(MetaClass tasksSpecMetaClass, List<ModificationTask> tasks, Binding binding) {
        tasksSpecMetaClass.setTargetPDelta = { String id, float delta ->
            tasks.add(new SetTargetPDelta(id, delta))
        }
    }
}
